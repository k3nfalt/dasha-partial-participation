...
\title{3-in-1: Computational Efficiency, Compressed Communication and Partial Participation in the Nonconvex Distributed Optimization}
...
The community developed a large number of unbiassed compressors, including Rand$K$ \citep{beznosikov2020biased, stich2018sparsified}, Adaptive sparsification \citep{wangni2018gradient} and Natural compression and dithering \citep{horvath2019natural}. We are aware of correlated compressors by \cite{szlendak2021permutation} and quantizers by \cite{suresh2022correlated} that help in the homogeneous regimes, but in this work, we are mainly concentrated on generic heterogeneous regimes, though, for simplicity, assume the independence of the compressors.
\begin{assumption}
...
The main goal of our paper is to develop a method for the nonconvex distributed optimization that will include three key features: computation efficiency, compressed communication, and partial participation.

\textbf{1. Computation efficiency}\\
...
% Typically, the researches provide partial participation as an ``free add-on'' to an existing method, \algname{PP-MARINA} in \citep{gorbunov2021marina}, \algname{EF21-PP} in


\section{Contributions}
% \alexander{Say somewhere that the compression can be done for free}

\section{Algorithm Description}

We now present \algname{DASHA-PP} (see Algorithm~\ref{alg:main_algorithm}), a family of methods to solve the optimization problem \eqref{eq:main_problem}. \algname{DASHA-PP} is based on \algname{DASHA}. 
Let us point out the main differences. i) if some node participates, then it does \algname{DASHA}-like steps; otherwise, it does nothing. ii) some steps are scaled by \nicefrac{1}{\probavailable} to preserve ``unbiasedness''. iii) In \algname{DASHA}, the momentum $b$ is used only to reduce the variance of stochastic gradients. In \algname{DASHA-PP}, the momentum $b$ plays the second role. It also helps to reduce the variance of ``partial participation randomness''. That is why it also appears in the gradient and finite-sum settings. iv) In the finite-sum setting, we present two methods: \algname{DASHA-PP-PAGE} and \algname{DASHA-PP-FINITE-MVR}. The former is based on \algname{PAGE} \citep{PAGE} and with small probability $\probpage$ calculates the full gradients of the functions $f_i$. The latter always calculates mini-batches, but it needs extra memory $\cO\left(d m\right)$ per node to store vectors $h^{t}_{ij}$. 
% v) the server in \algname{DASHA-PP} broadcasts two points $x^{t+1}$ and $x^{t},$ while the server in \algname{DASHA} broadcasts one point $x^{t+1}.$ 
One can easily show that \algname{DASHA-PP} reduces to \algname{DASHA} when $\probavailable = 1.$
...
\begin{algorithm}
  ...
  batch size $B$ (only in \algname{DASHA-PP-PAGE}, \algname{DASHA-PP-FINITE-MVR} and \algname{DASHA-PP-MVR}),
  ...
  \STATE Initialize $h^0_{ij}\in \R^d$ on the nodes and take $h^0_i = \frac{1}{m}\sum_{j=1}^m h^0_{ij}$ (only in \algname{DASHA-PP-FINITE-MVR})
  \FOR{$t = 0, 1, \dots, T - 1$}
...
\begin{algorithm}
  \caption{Calculate $k^{t+1}_i$ for \algname{DASHA-PP} in the gradient setting. See line~\ref{alg:calculate_k} in Alg.~\ref{alg:main_algorithm}}
...
\end{algorithm}
\begin{algorithm}
  \caption{Calculate $k^{t+1}_i$ for \algname{DASHA-PP-PAGE} in the finite-sum setting. See line~\ref{alg:calculate_k} in Alg.~\ref{alg:main_algorithm}}
...
\end{algorithm}
\begin{algorithm}
  \caption{Calculate $k^{t+1}_i$ for \algname{DASHA-PP-FINITE-MVR} in the finite-sum setting. See line~\ref{alg:calculate_k} in Alg.~\ref{alg:main_algorithm}}
...
\end{algorithm}
\begin{algorithm}
  \caption{Calculate $k^{t+1}_i$ for \algname{DASHA-PP-MVR} in the stochastic setting. See line~\ref{alg:calculate_k} in Alg.~\ref{alg:main_algorithm}}
...
\begin{restatable}{theorem}{CONVERGENCE}
  ...
  in Algorithm~\ref{alg:main_algorithm} \algname{(DASHA-PP)}, then $\Exp{\norm{\nabla f(\widehat{x}^T)}^2} \leq \frac{2 \left(f(x^0) - f^*\right)}{\gamma T}.$
  % \begin{align*}
...
Then \algname{DASHA-PP-PAGE}
    ...
    \begin{align}
      ...
      T \eqdef \cO\left(\frac{1}{\varepsilon}\left[L + \frac{\omega}{\probavailable\sqrt{n}}\left(\widehat{L} + \frac{L_{\max}}{\sqrt{B}}\right) + \frac{1}{\probavailable}\sqrt{\frac{m}{n}}\left(\frac{\widehat{L}}{\sqrt{B}} + \frac{L_{\max}}{B}\right)\right]\right)
    \end{align}
...
\begin{restatable}{theorem}{CONVERGENCEMVR}
  ...
  Then 
\begin{align*}
  &\Exp{\norm{\nabla f(\widehat{x}^T)}^2} \leq \frac{1}{T}\vast[\frac{2 \left(f(x^0) - f^*\right)}{\gamma} + \frac{2}{b} \norm{h^{0} - \nabla f(x^{0})}^2 \\
...
\begin{restatable}{corollary}{COROLLARYSTOCHASTIC}
  ...
  and $h^{0}_i = \frac{1}{B_{\textnormal{init}}} \sum_{k = 1}^{B_{\textnormal{init}}} \nabla f_i(x^0; \xi^0_{ik})$ for all $i \in [n],$ and batch size $B_{\textnormal{init}} = \Theta\left(\frac{\sqrt{\probavailable} B}{b}\right),$ then Algorithm~\ref{alg:main_algorithm} \algname{(DASHA-PP-MVR)} needs
  $$T \eqdef \cO\left(\frac{1}{\varepsilon}\left[L + \frac{\omega}{\probavailable\sqrt{n}}\left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right) + \frac{\sigma}{\probavailable \sqrt{\varepsilon} n}\left(\frac{\widehat{L}}{\sqrt{B}} + \frac{L_\sigma}{B}\right)\right] + \frac{\sigma^2}{\sqrt{\probavailable} n \varepsilon B}\right)$$
...
\begin{restatable}{corollary}{COROLLARYSTOCHASTICRANDK}
  Suppose that assumptions of Corollary~\ref{cor:stochastic} hold, batch size $B \leq \min\left\{\frac{\sigma}{\sqrt{\varepsilon} n}, \frac{L_{\sigma}^2}{\widehat{L}^2}\right\},$ we take Rand$K$ with $K = \zeta_{\cC} = \Theta\left(\frac{B d \sqrt{\varepsilon n}}{\sigma}\right).$ Then
  the communication complexity equals 
  \begin{align}
      \label{eq:complexity:stochastic:communication}
      \cO\left(\frac{d \sigma}{\sqrt{\probavailable} \sqrt{n \varepsilon}} + \frac{L_{\sigma}d}{\probavailable \sqrt{n} \varepsilon}\right),
  \end{align}
  and the expected \# of stochastic gradient calculations per node equals
  \begin{align}
      \label{eq:complexity:stochastic_oracle}
      \cO\left(\frac{\sigma^2}{\sqrt{\probavailable} n \varepsilon} + \frac{L_{\sigma} \sigma}{\probavailable \varepsilon^{\nicefrac{3}{2}} n}\right).
  \end{align}
\end{restatable}

\alexander{For $\probavailable = 1$ is DASHA-MVR}
\alexander{TODO: Prove it!}
...
\section{The Problem of Compressed Vectors in the Stochastic Setting}

\section{Experiments}

In experiments, our main goal is to compare \algname{DASHA-PP} with \algname{DASHA}. We verify that the convergence rates of \algname{DASHA-PP} no worse than times $\nicefrac{1}{\probavailable}$. We use the standard setting in experiments where all parameters except step sizes are taken as suggested in theory. Step sizes are finetuned from a set~$\{2^{i}\,|\,i \in [-10, 10]\}.$
...
\begin{proof}
  ...
  \begin{proof}
    ...
    \begin{align*}
        ...
        &\leq \frac{1}{T}\vast[2 \left(f(x^0) - f^*\right)\left(L + \sqrt{\frac{48 \omega (2 \omega + 1)}{n \probavailable^2} \left(\widehat{L}^2 + \frac{(1 - b)^2 L_{\sigma}^2}{B}\right) + \frac{12}{n \probavailable b}\left(\left(1 - \frac{\probpairaa}{\probavailable}\right) \widehat{L}^2 + \frac{(1 - b)^2 L_{\sigma}^2}{B}\right)}\right) \\
    ...
    \begin{align*}
        &\Exp{\norm{\nabla f(\widehat{x}^T)}^2} \\
        &= \cO\vast(\frac{1}{T}\vast[\left(L + \frac{\omega}{\probavailable \sqrt{n}} \left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right) + \sqrt{\frac{\sigma^2}{\probavailable^2 \varepsilon n^2 B}}\left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right)\right)\\
    ...
    \begin{align*}
        &T=\cO\vast(\frac{1}{\varepsilon}\vast[L + \frac{\omega}{\probavailable \sqrt{n}} \left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right) + \sqrt{\frac{\sigma^2}{\probavailable^2 \varepsilon n^2 B}}\left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right)\\
    ...
    \begin{align*}
      ...
      &=\cO\vast(\frac{1}{\varepsilon}\vast[L + \frac{\omega}{\probavailable \sqrt{n}} \left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right) + \sqrt{\frac{\sigma^2}{\probavailable^2 \varepsilon n^2 B}}\left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right)\\
      &\qquad + \frac{\sigma^2}{\sqrt{\probavailable} n B}  + \frac{\varepsilon}{\sqrt{\probavailable}}\vast]\vast) \\
      &=\cO\vast(\frac{1}{\varepsilon}\vast[L + \frac{\omega}{\probavailable \sqrt{n}} \left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right) + \sqrt{\frac{\sigma^2}{\probavailable^2 \varepsilon n^2 B}}\left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right)\vast] + \frac{\sigma^2}{\sqrt{\probavailable} n \varepsilon B}  + \frac{1}{\sqrt{\probavailable}} \vast).
    \end{align*}

    Using the assumption $\frac{\sigma^2}{n \varepsilon B} \geq 1,$ we can conclude the proof of the inequality. The number of stochastic gradients that each node calculates equals $B_{\textnormal{init}} + 2BT = \cO(B_{\textnormal{init}} + BT).$
\end{proof}

\COROLLARYSTOCHASTICRANDK*

\begin{proof}
  \alexander{Add RandK theorem}
  The communication complexity equals
  \begin{eqnarray*}
      \cO\left(d + \zeta_{\cC} T\right) &=& \cO\left(d + \frac{1}{\varepsilon}\vast[K L + K\frac{\omega}{\probavailable \sqrt{n}} \left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right) + K \sqrt{\frac{\sigma^2}{\probavailable^2 \varepsilon n^2 B}}\left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right)\vast] + K\frac{\sigma^2}{\sqrt{\probavailable} n \varepsilon B}\right).
  \end{eqnarray*}
  Due to $B \leq \frac{L_{\sigma}^2}{\widehat{L}^2}, $ we have $\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}} \leq \frac{2L_{\sigma}}{\sqrt{B}}$ and
  \begin{eqnarray*}
    \cO\left(d + \zeta_{\cC} T\right) &=& \cO\left(d + \frac{1}{\varepsilon}\vast[K L + K\frac{\omega}{\probavailable \sqrt{n}} \left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right) + K \sqrt{\frac{\sigma^2}{\probavailable^2 \varepsilon n^2 B}}\frac{L_{\sigma}}{\sqrt{B}}\vast] + K\frac{\sigma^2}{\sqrt{\probavailable} n \varepsilon B}\right).
\end{eqnarray*}
  ...
  \begin{eqnarray*}
      \cO\left(d + \zeta_{\cC} T\right) &=& \cO\left(d + \frac{1}{\varepsilon}\vast[\frac{d}{\sqrt{n}}L + \frac{d}{\probavailable \sqrt{n}} \left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right) + \frac{d}{\probavailable \sqrt{n}} L_{\sigma}\vast] + \frac{d \sigma}{\sqrt{\probavailable} \sqrt{n \varepsilon}}\right) \\
      &=& \cO\left(\frac{d \sigma}{\sqrt{\probavailable} \sqrt{n \varepsilon}} + \frac{L_{\sigma}d}{\probavailable \sqrt{n} \varepsilon}\right) \\
  \end{eqnarray*}
  And the expected number of stochastic gradient calculations per node equals
  \begin{align*}
      &\cO\left(B_{\textnormal{init}} + B T\right) \\
      &= \cO\left(\frac{\sigma^2}{\sqrt{\probavailable} n \varepsilon} +  \frac{B \omega}{\sqrt{\probavailable}} \sqrt{\frac{\sigma^2}{n \varepsilon B}} + \frac{1}{\varepsilon}\vast[B L + B\frac{\omega}{\probavailable \sqrt{n}} \left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right) + B \sqrt{\frac{\sigma^2}{\probavailable^2 \varepsilon n^2 B}}\left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right)\vast] + B\frac{\sigma^2}{\sqrt{\probavailable} n \varepsilon B}\right) \\
      &= \cO\left(\frac{\sigma^2}{\sqrt{\probavailable} n \varepsilon} +  \frac{B d}{K \sqrt{\probavailable}} \sqrt{\frac{\sigma^2}{n \varepsilon B}} + \frac{1}{\varepsilon}\vast[B L + B\frac{d}{K \probavailable \sqrt{n}} \left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right) + B \sqrt{\frac{\sigma^2}{\probavailable^2 \varepsilon n^2 B}}\frac{L_{\sigma}}{\sqrt{B}}\vast] + \frac{\sigma^2}{\sqrt{\probavailable} n \varepsilon}\right) \\
      &= \cO\left(\frac{\sigma^2}{\sqrt{\probavailable} n \varepsilon} + \frac{\sigma^2}{\sqrt{\probavailable} n \varepsilon \sqrt{B}} + \frac{1}{\varepsilon}\vast[\frac{\sigma}{\sqrt{\varepsilon} n} L + \frac{\sigma}{\probavailable \sqrt{\varepsilon} n} \left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right) + \frac{\sigma}{\probavailable \sqrt{\varepsilon} n}L_{\sigma}\vast]\right) \\
      &= \cO\left(\frac{\sigma^2}{\sqrt{\probavailable} n \varepsilon} + \frac{L_{\sigma} \sigma}{\probavailable \varepsilon^{\nicefrac{3}{2}} n}\right).
  \end{align*}
\end{proof}

  \newpage
  \section{Description of \algname{DASHA-PP-SYNC-MVR}}
  ...
  % In this section, we provide a description of \algname{\algorithmname-SYNC-MVR} (see Algorithm~\ref{alg:main_algorithm_mvr_sync}). This algorithm is closely related to \algname{\algorithmname-MVR} (Algorithm~\ref{alg:main_algorithm}), but \algname{\algorithmname-SYNC-MVR} synchronizes all nodes with some probability $p$. This synchronization procedure enabled us to fix the convergence rate suboptimality of \algname{\algorithmname-MVR} w.r.t.\,$\omega$.
   ...
   By analogy to \citep{tyurin2022dasha}, we provide a ``synchronized'' version of the algorithm. With a small probability, participating nodes calculate and send a mega batch without compression. This helps us to resolve the suboptimality of \algname{DASHA-PP-MVR} w.r.t.\,$\omega.$
  
  \begin{algorithm}[h]
      \caption{\algname{DASHA-PP-SYNC-MVR}}
  ...
  In the following theorem, we provide the convergence rate of \algname{DASHA-PP-SYNC-MVR}.
  ...
  \begin{restatable}{corollary}{COROLLARYSYNCSTOCHASTIC}
    ...
    % $B_{\textnormal{init}} = ?,$ 
    then \algname{DASHA-PP-SYNC-MVR}
    needs
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\quad + \left(\frac{2 \nu b^2}{n \probmega \probavailable} + \frac{2 \rho b^2}{\probavailable \probmega}\right) \frac{\sigma^2}{B'},
  \end{align*}
  where simplified a term using $\probpairaa \geq 0.$ Let us take $\nu = \frac{\gamma}{b}$ to obtain
  \begin{align*}
    &\Exp{f(x^{t + 1})} + \frac{\gamma \left(2 \omega + 1\right)}{\probavailable} \Exp{\norm{g^{t+1} - h^{t+1}}^2} + \frac{\gamma\left(\left(2 \omega + 1\right)\probavailable - \probpairaa\right)}{n \probavailable^2} \Exp{\frac{1}{n}\sum_{i=1}^n\norm{g^{t+1}_i - h^{t+1}_i}^2}\\
    &\quad  + \frac{\gamma}{b} \Exp{\norm{h^{t+1} - \nabla f(x^{t+1})}^2} + \rho \Exp{\frac{1}{n}\sum_{i=1}^n\norm{h^{t+1}_i - \nabla f_i(x^{t+1})}^2}\\
    &\leq \Exp{f(x^t)} - \frac{\gamma}{2}\Exp{\norm{\nabla f(x^t)}^2} \\
...
\begin{proof}
  ...
  \begin{align*}
    \Exp{\norm{\nabla f(\widehat{x}^T)}^2} &\leq \frac{1}{T}\vast[2 \left(f(x^0) - f^*\right)\left(L + \sqrt{\frac{8 \left(2\omega + 1\right) \omega}{n \probavailable^2}\left(\widehat{L}^2 + \frac{L_{\sigma}^2}{B}\right) + \frac{8}{n \probmega \probavailable^2} \left(\left(1 - \frac{\probpairaa}{\probavailable}\right) \widehat{L}^2 + \frac{L_{\sigma}^2}{B}\right)}\right) \\
  ...
  \begin{align*}
    T = \cO\left(\frac{1}{\varepsilon}\vast[\left(L + \sqrt{\frac{\omega^2}{n \probavailable^2}\left(\widehat{L}^2 + \frac{L_{\sigma}^2}{B}\right) + \frac{1}{n \probmega \probavailable^2} \left(\widehat{L}^2 + \frac{L_{\sigma}^2}{B}\right)}\right) + \frac{\sigma^2}{n \probmega \probavailable B_{\textnormal{init}}} \vast]\right)
  \end{align*}
  Considering the choice of $\probmega$ and $B_{\textnormal{init}},$ we obtain
  \begin{align*}
    T &=\cO\left(\frac{1}{\varepsilon}\vast[\left(L + \left(\frac{\omega}{\probavailable\sqrt{n}} + \sqrt{\frac{d}{\probavailable^2 \zeta_{\cC} n}}\right) \left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right)  + \frac{\sigma}{\probavailable \sqrt{\varepsilon} n} \left(\frac{\widehat{L}}{\sqrt{B}} + \frac{L_{\sigma}}{B}\right)\right) + \frac{\sigma^2}{n \probmega \probavailable B_{\textnormal{init}}} \vast]\right) \\
    &=\cO\left(\frac{1}{\varepsilon}\vast[L + \left(\frac{\omega}{\probavailable\sqrt{n}} + \sqrt{\frac{d}{\probavailable^2 \zeta_{\cC} n}}\right) \left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right)  + \frac{\sigma}{\probavailable \sqrt{\varepsilon} n} \left(\frac{\widehat{L}}{\sqrt{B}} + \frac{L_{\sigma}}{B}\right)\vast] + \frac{\sigma^2}{\sqrt{\probavailable} n \varepsilon B}\right).
  \end{align*}
\end{proof}

% \probmega = \min \left\{\ \frac{\zeta_{\cC}}{d}, \frac{n \varepsilon B}{\sigma^2}\right\}
