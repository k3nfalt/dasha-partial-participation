...
Upgrading \algname{DASHA} is not an exercise, let us point out the main differences: 
i) The theoretical analysis of \algname{DASHA-PP} is more complicated: while in \algname{DASHA}, the randomness from compressors is independent of the randomness from stochastic gradients, in \algname{DASHA-PP}, these two randomnesses are coupled by the randomness from the partial participation. Moreover, the new method reduces the variance from partial participation. 
% The theoretical analysis of \algname{DASHA-PP} is more complicated: while in \algname{DASHA}, the randomness from compressors is conditionally independent from the randomness from stochastic gradients 
ii) if some node participates, then it does \algname{DASHA}-like steps; otherwise, it does nothing. 
iii) some steps are scaled by \nicefrac{1}{\probavailable} to preserve ``unbiasedness''. iv) In \algname{DASHA}, the momentum $b$ is used only to reduce the variance of stochastic gradients. In \algname{DASHA-PP}, the momentum $b$ plays the second role. It also helps to reduce the variance of partial participation randomness. That is why it also appears in the gradient and finite-sum settings. 
iv) In the finite-sum setting, we present two methods: \algname{DASHA-PP-PAGE} and \algname{DASHA-PP-FINITE-MVR}. The former is based on \algname{PAGE} \citep{PAGE} and with small probability $\probpage$ calculates the full gradients of the functions $f_i$. The latter always calculates mini-batches, but it needs extra memory $\cO\left(d m\right)$ per node to store vectors $h^{t}_{ij}$.
...
It is left to provide the convergence rate of \algname{DASHA-PP-FINITE-MVR}. The conclusions are the same for the method.
...
\alexander{Better to delete the next claim.}
...
In the following corollary, we consider Rand$K$ compressors and show that with the particular choice of parameters, up to the Lipschitz constants and probability $\probavailable$ factor, \algname{DASHA-PP-MVR} gets the optimal oracle complexity and SOTA communication complexity.

\begin{restatable}{corollary}{COROLLARYSTOCHASTICRANDK}
  Suppose that assumptions of Corollary~\ref{cor:stochastic} hold, batch size $B \leq \min\left\{\frac{\sigma}{\sqrt{\varepsilon} n}, \frac{L_{\sigma}^2}{\mathbbm{1}_{\probavailable}^2 \widehat{L}^2}\right\},$\footnote{If $\mathbbm{1}_{\probavailable} = 0,$ then $\frac{L_{\sigma}^2}{\mathbbm{1}_{\probavailable}^2 \widehat{L}^2} = +\infty$} we take Rand$K$ with $K = \zeta_{\cC} = \Theta\left(\frac{B d \sqrt{\varepsilon n}}{\sigma}\right).$ Then
...
\begin{proof}
  ...
  % One only should note that the expected number of gradients calculations at each communication round equals $p m + (1 - p) B = \frac{2 m B}{m + B} \leq 2 B.$
  \alexander{Add lemma for RandK}
