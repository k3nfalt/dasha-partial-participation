...
We now present \algname{DASHA-PP} (see Algorithm~\ref{alg:main_algorithm}), a family of methods to solve the optimization problem \eqref{eq:main_problem}. \algname{DASHA-PP} is based on \algname{DASHA} by \cite{tyurin2022dasha}. 
...
\begin{restatable}{corollary}{COROLLARYSTOCHASTICRANDK}
  ...
  Suppose that assumptions of Corollary~\ref{cor:stochastic} hold, batch size $B \leq \min\left\{\frac{\sigma}{\sqrt{\varepsilon} n}, \frac{L_{\sigma}^2}{\mathbbm{1}_{\probavailable}^2 \widehat{L}^2}\right\},$ we take Rand$K$ with $K = \Theta\left(\frac{B d \sqrt{\varepsilon n}}{\sigma}\right).$ Then
...
We are aware that the initial batch size $B_{\textnormal{init}}$ can be suboptimal w.r.t.\,$\omega$ in \algname{DASHA-PP-MVR} in some regimes (see also \citep{tyurin2022dasha}). However, Corollary~\ref{cor:stochastic:randk} reveals that we can escape these regimes by choosing the parameter $K$ of Rand$K$ in a particular way. To get the complete picture, we provide \algname{DASHA-PP-SYNC-MVR} in the appendix.
...
In experiments, our main goal is to compare \algname{DASHA-PP} with \algname{DASHA}. 
We verify that the bigger $\probavailable$, the closer \algname{DASHA-PP} is to \algname{DASHA}.
% We verify that the convergence rates of \algname{DASHA-PP} no worse than times $\nicefrac{1}{\probavailable}$. 
We use the standard setting in experiments where all parameters except step sizes are taken as suggested in theory. Step sizes are finetuned from a set~$\{2^{i}\,|\,i \in [-10, 10]\}.$
...
\newpage
\section{Analysis of \algname{DASHA-PP} under Polyak-\L ojasiewicz Condition}
\label{sec:pl_condition}

In this section, we provide the theoretical convergence rates of \algname{DASHA-PP} under Polyak-\L ojasiewiczc Condition.
...
\subsection{Standard Lemma under Polyak-\L ojasiewicz Condition}
...
\newpage
  ...
  % In this section, we provide a description of \algname{\algorithmname-SYNC-MVR} (see Algorithm~\ref{alg:main_algorithm_mvr_sync}). This algorithm is closely related to \algname{\algorithmname-MVR} (Algorithm~\ref{alg:main_algorithm}), but \algname{\algorithmname-SYNC-MVR} synchronizes all nodes with some probability $p$. This synchronization procedure enabled us to fix the convergence rate suboptimality of \algname{\algorithmname-MVR} w.r.t.\,$\omega$.
   ...
   By analogy to \citep{tyurin2022dasha}, we provide a ``synchronized'' version of the algorithm. With a small probability, participating nodes calculate and send a mega batch without compression. This helps us to resolve the suboptimality of \algname{DASHA-PP-MVR} w.r.t.\,$\omega.$ Note that this suboptimality is not a problem. 
   We show in Corollary~\ref{cor:stochastic:randk} that \algname{DASHA-PP-MVR} can have the optimal oracle complexity and SOTA communication complexity with the particular choices of parameters.
  ...
  The main improvement of Corollary~\ref{cor:sync_stochastic} over Corollary~\ref{cor:stochastic} is the size of the initial batch size $B_{\textnormal{init}}$. However, Corollary~\ref{cor:stochastic:randk} reveals that we can avoid regimes when \algname{DASHA-PP-MVR} is suboptimal.
