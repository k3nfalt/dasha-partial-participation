...
In the following corollary, we consider Rand$K$ compressors and show that with the particular choice of parameters, up to Lipchitz constants and probability $\probavailable$ factors, \algname{DASHA-PP-MVR} gets the optimal oracle complexity and SOTA communication complexity.

\begin{restatable}{corollary}{COROLLARYSTOCHASTICRANDK}
  Suppose that assumptions of Corollary~\ref{cor:stochastic} hold, batch size $B \leq \min\left\{\frac{\sigma}{\sqrt{\varepsilon} n}, \frac{L_{\sigma}^2}{\mathbbm{1}_{\probavailable}^2 \widehat{L}^2}\right\},$ we take Rand$K$ with $K = \zeta_{\cC} = \Theta\left(\frac{B d \sqrt{\varepsilon n}}{\sigma}\right).$ Then
