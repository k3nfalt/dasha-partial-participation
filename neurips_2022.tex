...
\alexander{Regimes when sync if better are not practical???}
  ...
  \begin{algorithm}[h]
      ...
      \FOR{$i = 1, \dots, n$ in parallel}
          \IF{$i^{\textnormal{th}} \textnormal{ node is \textit{participating}}$\textsuperscript{\red (a)}}
              ...
              \STATE $h^{t+1}_i = h^t_i + \frac{1}{\probavailable}k^{t+1}_i$ 
              \STATE $g^{t+1}_i = g^{t}_i + \frac{1}{\probavailable}k^{t+1}_i - \frac{1}{\probavailable} \left(g^t_i - h^t_i\right)$
              \STATE $m^{t+1}_i = g^{t+1}_i$ 
          \ELSE
              \STATE Generate i.i.d.\,samples $\{\xi^{t+1}_{ij}\}_{j=1}^B$ of size $B$ from $\mathcal{D}_i.$
      ...
      \ENDFOR
      \IF{$c^{t+1} = 1$}
          \STATE $g^{t+1} = \frac{1}{n} \sum_{i=1}^{n} m^{t+1}_i$
      \ELSE
          \STATE $g^{t+1} = g^t + \frac{1}{n} \sum_{i=1}^{n} m^{t+1}_i$
      \ENDIF
      \ENDFOR
...
\begin{align*}
    ...
    &g^{t+1}_{i,1} \eqdef \begin{cases}
      g^{t}_i + \frac{1}{\probavailable}k^{t+1}_{i, 1} - \frac{1}{\probavailable} \left(g^t_i - h^t_i\right),& i^{\textnormal{th}} \textnormal{ node is \textit{participating}}, \\
...
\begin{lemma}
  ...
  \begin{align*}
    ...
    &\leq \frac{2 \left(1 - \probmega\right) \omega}{n^2 \probavailable}\sum_{i=1}^n\norm{k^{t+1}_{i, 2}}^2 + \frac{\probmega \left(\probavailable - \probpairaa\right) + \left(1 - \probmega\right)a^2\left(\left(2\omega + 1\right)\probavailable - \probpairaa\right)}{n^2 \probavailable^2}\sum_{i=1}^n\norm{g^{t}_i - h^{t}_i}^2 \\
    &\quad + \left(1 - \probmega\right) (1 - a)^2 \norm{g^{t} - h^{t}}^2,
  \end{align*}
  and
  \begin{align*}
    &\ExpSub{\cC}{\ExpSub{\probavailable}{\ExpSub{\probmega}{\norm{g^{t+1}_i - h^{t+1}_i}^2}}} \\
    &\leq \frac{2\left(1 - \probmega\right) \omega}{\probavailable} \norm{k^{t+1}_{i, 2}}^2 + \frac{\probmega (1 - \probavailable) + \left(1 - \probmega\right) \left(2\omega + 1 - \probavailable\right) a^2}{\probavailable} \norm{g^{t}_{i} - h^{t}_{i}}^2 \\
...
\begin{proof}
  ...
  Using
  \begin{align*}
    \ExpSub{\probavailable}{g^{t+1}_{i,1} - h^{t+1}_{i,1}} = g^{t}_i + k^{t+1}_{i, 1} - \left(g^t_i - h^t_i\right) - h^t_i - k^{t+1}_{i, 1} = 0
  \end{align*}
  and 
  \begin{align*}
  ...
  \begin{align*}
    &\ExpSub{\cC}{\ExpSub{\probavailable}{\ExpSub{\probmega}{\norm{g^{t+1} - h^{t+1}}^2}}} \\
    &\overset{\eqref{auxiliary:variance_decomposition}}{=} \probmega \ExpSub{\probavailable}{\norm{g^{t+1}_1 - h^{t+1}_1 - \ExpSub{\probavailable}{g^{t+1}_1 - h^{t+1}_1}}^2} + \left(1 - \probmega\right) \ExpSub{\cC}{\ExpSub{\probavailable}{\norm{g^{t+1}_2 - h^{t+1}_2}^2}} \\
    &\quad + \left(1 - \probmega\right) (1 - a)^2 \norm{g^{t} - h^{t}}^2.
  \end{align*}
  We can use Lemma~\ref{lemma:sampling} two times with i) $r_i = g^{t}_i - h^{t}_i$ and $s_i = g^{t}_i - h^{t}_i$ and ii) $r_i = g^{t}_i - h^{t}_i$ and $s_i = \probavailable\cC_i\left(\frac{1}{\probavailable}k^{t+1}_{i, 2} - \frac{a}{\probavailable} \left(g^t_i - h^{t}_i\right)\right) - k^{t+1}_{i, 2}$, to obtain
  \begin{align*}
    &\ExpSub{\cC}{\ExpSub{\probavailable}{\ExpSub{\probmega}{\norm{g^{t+1} - h^{t+1}}^2}}} \\
    &\leq \frac{\probmega\left(\probavailable - \probpairaa\right)}{n^2 \probavailable^2}\sum_{i=1}^n\norm{g^{t}_i - h^{t}_i}^2 \\
    ...
    &\quad + \left(1 - \probmega\right) (1 - a)^2 \norm{g^{t} - h^{t}}^2 \\
    &= \frac{\left(\probmega + \left(1 - \probmega\right)a^2\right)\left(\probavailable - \probpairaa\right)}{n^2 \probavailable^2}\sum_{i=1}^n\norm{g^{t}_i - h^{t}_i}^2 \\
    ...
    &\quad + \left(1 - \probmega\right) (1 - a)^2 \norm{g^{t} - h^{t}}^2 \\
    &\leq \frac{\left(\probmega + \left(1 - \probmega\right)a^2\right)\left(\probavailable - \probpairaa\right)}{n^2 \probavailable^2}\sum_{i=1}^n\norm{g^{t}_i - h^{t}_i}^2 \\
  ...
  \begin{align*}
    ...
    &= \probmega \left(\frac{(1 - \probavailable)^2}{\probavailable} \norm{g^{t}_{i} - h^{t}_{i}}^2 + \left(1 - \probavailable\right) \norm{g^{t}_{i} - h^{t}_{i}}^2\right) + \left(1 - \probmega\right) \ExpSub{\cC}{\ExpSub{\probavailable}{\norm{g^{t+1}_{i,2} - h^{t+1}_{i,2}}^2}}\\
    &= \frac{\probmega (1 - \probavailable)}{\probavailable} \norm{g^{t}_{i} - h^{t}_{i}}^2 + \left(1 - \probmega\right) \ExpSub{\cC}{\ExpSub{\probavailable}{\norm{g^{t+1}_{i,2} - h^{t+1}_{i,2}}^2}}\\
    &\overset{\eqref{auxiliary:variance_decomposition}}{=} \frac{\probmega (1 - \probavailable)}{\probavailable} \norm{g^{t}_{i} - h^{t}_{i}}^2 + \left(1 - \probmega\right) \ExpSub{\cC}{\ExpSub{\probavailable}{\norm{g^{t+1}_{i,2} - h^{t+1}_{i,2} - (1 - a)\left(g^{t}_{i} - h^{t}_{i}\right)}^2}}\\
    &\quad + \left(1 - \probmega\right) (1 - a)^2 \norm{g^{t}_{i} - h^{t}_{i}}^2 \\
    &= \frac{\probmega (1 - \probavailable)}{\probavailable} \norm{g^{t}_{i} - h^{t}_{i}}^2 \\
    ...
    &\quad + \left(1 - \probmega\right) (1 - a)^2 \norm{g^{t}_{i} - h^{t}_{i}}^2 \\
    &\overset{\eqref{auxiliary:variance_decomposition}}{=} \frac{\probmega (1 - \probavailable) + \left(1 - \probmega\right) \left(1 - \probavailable\right) a^2}{\probavailable} \norm{g^{t}_{i} - h^{t}_{i}}^2 \\
    ...
    &\quad + \left(1 - \probmega\right) (1 - a)^2 \norm{g^{t}_{i} - h^{t}_{i}}^2 \\
    &\leq \frac{\probmega (1 - \probavailable) + \left(1 - \probmega\right) \left(1 - \probavailable\right) a^2}{\probavailable} \norm{g^{t}_{i} - h^{t}_{i}}^2 \\
...
\begin{restatable}{theorem}{CONVERGENCESYNCMVR}
  ...
  Suppose that Assumptions \ref{ass:lower_bound}, \ref{ass:lipschitz_constant}, \ref{ass:nodes_lipschitz_constant}, \ref{ass:stochastic_unbiased_and_variance_bounded}, \ref{ass:mean_square_smoothness}, \ref{ass:compressors} and \ref{ass:partial_participation} hold. Let us take $a = $, probability $\probmega \in (0, 1],$ batch size $B' \geq B \geq 1$ $$\gamma \leq \left(L + \sqrt{\frac{12 \omega (2\omega + 1) (1 - p)}{n}\left(\frac{L_{\sigma}^2}{B} + \widehat{L}^2\right) + \frac{2 (1 - p) L_\sigma^2}{p n B}}\right)^{-1},$$ and $h^{0}_i = g^{0}_i$ for all $i \in [n]$
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\qquad\quad+ \frac{\probmega \left(\probavailable - \probpairaa\right) + \left(1 - \probmega\right)a^2\left(\left(2\omega + 1\right)\probavailable - \probpairaa\right)}{n^2 \probavailable^2}\sum_{i=1}^n\norm{g^{t}_i - h^{t}_i}^2\\
      &\qquad\quad + \left(1 - \probmega\right) (1 - a)^2 \norm{g^{t} - h^{t}}^2\Bigg) \\
      &\quad  + \eta {\rm E}\Bigg(\frac{2\left(1 - \probmega\right) \omega}{\probavailable} \left(\frac{L_{\sigma}^2}{B} + \widehat{L}^2\right) \norm{x^{t+1} - x^{t}}^2 \\
      &\qquad\quad+ \frac{\probmega (1 - \probavailable) + \left(1 - \probmega\right) \left(2\omega + 1 - \probavailable\right) a^2}{n \probavailable} \sum_{i=1}^n \norm{g^{t}_{i} - h^{t}_{i}}^2\\
      &\qquad\quad + \left(1 - \probmega\right) (1 - a)^2 \frac{1}{n} \sum_{i=1}^n \norm{g^{t}_{i} - h^{t}_{i}}^2\Bigg) \\
  ...
  \end{align*}
  \alexander{Check 2nd inequality}
  Let us simplify the last inequality. Since $B' \geq B$ and $b \leq \frac{\probmega \probavailable}{2 - \probavailable} \leq \probmega,$ we have $$\frac{2 \probmega L_{\sigma}^2}{\probavailable B'}\left(1 - \frac{b}{\probmega}\right)^2 \leq \frac{2 \probmega L_{\sigma}^2}{\probavailable B},$$ $$\left(\probmega \left(1 - \frac{b}{\probmega}\right)^2 + (1 - \probmega)\right) \leq 1 - b,$$ and $$\left(\frac{2\left(1 - \probavailable\right)b^2}{n \probmega \probavailable} + \probmega \left(1 - \frac{b}{\probmega}\right)^2 + (1 - \probmega)\right) \leq 1 - b.$$
  ...
  \begin{align*}
    ...
    &\quad  + \nu \Exp{\norm{h^{t+1} - \nabla f(x^{t+1})}^2} + \rho \Exp{\frac{1}{n}\sum_{i=1}^n\norm{h^{t+1}_i - \nabla f_i(x^{t+1})}^2}\\
    &\leq \Exp{f(x^t)} - \frac{\gamma}{2}\Exp{\norm{\nabla f(x^t)}^2} \\
    &\quad - \Bigg(\frac{1}{2\gamma} - \frac{L}{2} - \frac{2 \kappa \left(1 - \probmega\right) \omega}{n \probavailable}\left(\frac{L_{\sigma}^2}{B} + \widehat{L}^2\right) - \frac{2\eta \left(1 - \probmega\right) \omega}{\probavailable} \left(\frac{L_{\sigma}^2}{B} + \widehat{L}^2\right) \\
    ...
    &\quad + \left(\gamma + \kappa \left(1 - \probmega\right)\left(1 - a\right)^2\right) \Exp{\norm{g^{t} - h^{t}}^2} \\
    &\quad + \Bigg(\kappa \frac{\probmega \left(\probavailable - \probpairaa\right) + \left(1 - \probmega\right)a^2\left(\left(2\omega + 1\right)\probavailable - \probpairaa\right)}{n^2 \probavailable^2}\\
    &\qquad\quad \eta \left(\frac{\probmega (1 - \probavailable) + \left(1 - \probmega\right) \left(2\omega + 1 - \probavailable\right) a^2}{n \probavailable} + \left(1 - \probmega\right) (1 - a)^2\right)\Bigg)\Exp{\frac{1}{n}\sum_{i=1}^n\norm{g^{t}_i - h^{t}_i}^2} \\
    &\quad + \left(\gamma + \nu \left(1 - b\right)\right) \Exp{\norm{h^{t} - \nabla f(x^{t})}^2} \\
    &\quad + \Bigg(\nu \frac{2\left(\probavailable - \probpairaa\right) b^2}{n^2 \probavailable^2 \probmega} + \rho (1 - b)\Bigg)\Exp{\frac{1}{n}\sum_{i=1}^n\norm{h^{t}_i - \nabla f_i(x^{t})}^2} \\
