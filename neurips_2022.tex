...
However, previously proposed methods have limitations: i) methods from \citep{MARINA, zhao2021fedpage} still require synchronization of all nodes with a small probability, ii) in the stochastic settings, proposed methods with the partial participation mechanism \citep{tyurin2022dasha, zhao2021faster} provide results without variance reduction techniques from \citep{SPIDER, PAGE, cutkosky2019momentum} and, therefore, get suboptimal oracle complexities, iii) in the finite-sum setting, the work by \cite{li2021zerosarah} focuses on the homogeneous regime only (the functions $f_i$ are equal). iv) The paper by \cite{karimireddy2020mime} considers the online version of the problem \eqref{eq:main_problem}. Therefore, \cite{karimireddy2020mime} require the bounded inter-client gradient variance assumption, which is strict. Also, their method calculates the full gradient in every communication round.
...
\begin{enumerate}
  ...
  \item Did you describe the limitations of your work?
    \answerYes{for instance, see Section~\ref{sec:partial_participation_sampling}}
...
\begin{enumerate}
  \item Did you state the full set of assumptions of all theoretical results?
    \answerYes{see Section~\ref{sec:opt_problem}}
...
\begin{enumerate}
  ...
  \item Did you specify all the training details (e.g., data splits, hyperparameters, how they were chosen)?
    \answerYes{see Section~\ref{sec:experiments} and Section~\ref{sec:experiments_details}}
	\item Did you report error bars (e.g., with respect to the random seed after running experiments multiple times)?
    \answerNo{}
	\item Did you include the total amount of compute and the type of resources used (e.g., type of GPUs, internal cluster, or cloud provider)?
    \answerYes{see Section~\ref{sec:experiments_details}}
\end{enumerate}
...
In all experiments, we take the \textit{real-sim} dataset with dimension $d = \num[group-separator={,}]{20958}$ and the number of samples equals $\num[group-separator={,}]{72309}$ from LIBSVM datasets \cite{chang2011libsvm} (under the 3-clause BSD license), and randomly split the dataset between $n = 100$ nodes equally, ignoring residual samples.
