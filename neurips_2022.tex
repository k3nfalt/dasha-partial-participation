...
It is left to provide the convergence rate of \algname{\algorithmname-FINITE-MVR}. The conclusions are the same for the method.
...
We are aware that the initial batch size $B_{\textnormal{init}}$ can be suboptimal w.r.t.\,$\omega$ in \algname{\algorithmname-MVR} in some regimes (see also \citep{tyurin2022dasha}). However, Corollary~\ref{cor:stochastic:randk} reveals that we can escape these regimes by choosing the parameter $K$ of Rand$K$ in a particular way. To get the complete picture, we provide \algname{\algorithmname-SYNC-MVR} in the appendix.
...
We now provide the example to explain why the only choice of $B = \cO\left(\nicefrac{L_{\max}^2}{\widehat{L}^2}\right)$ and $B = \cO\left(\nicefrac{L_{\sigma}^2}{\widehat{L}^2}\right)$ in \algname{\algorithmname-PAGE} and \algname{\algorithmname-MVR}, accordingly, guarantees the degeneration up to $\nicefrac{1}{\probavailable}.$
...
Now, we discuss the obtained bounds. Let us provide bounds up to logarithmic factors and use $\widetilde{\cO}\left(\cdot\right)$ notation. The provided theorems states that to get $\varepsilon$-solution \algname{\algorithmname} have to run
\begin{align}
  \label{eq:pl_compare:new:gradient}
...
\end{align}
communication rounds in gradient, finite-sum and stochastic settings, accordingly. In the stochastic setting, we take $b = \Theta\left(\min \left\{\ \frac{\probavailable}{\omega} \sqrt{\frac{\mu n \varepsilon B}{\sigma^2}}, \frac{\probavailable \mu n \varepsilon B}{\sigma^2}\right\}\right) \geq \Theta\left(\min \left\{\ \frac{\probavailable}{\omega^2}, \frac{\probavailable \mu n \varepsilon B}{\sigma^2}\right\}\right).$

The methods \algname{DASHA}, \algname{DASHA-PAGE}, \algname{DASHA-SYNC-MVR} from \algname{DASHA} \citep{tyurin2022dasha}, have to run
\begin{align}
  \label{eq:pl_compare:old:gradient}
  \widetilde{\cO}\left(\omega + \frac{L}{\mu} + \frac{\omega \widehat{L}}{\mu \sqrt{n}}\right),
\end{align}
\begin{align}
  \label{eq:pl_compare:old:page}
...
\end{align}
communication rounds in gradient, finite-sum and stochastic settings\footnote{For simplicity, we omitted $\frac{d}{\zeta_{\cC}}$ term from the complexity in the stochastic setting, where $\zeta_{\cC}$ is defined in Definition~\ref{def:expected_density}. For instance, for the Rand$K$ compressor (see Definition~\ref{def:rand_k} and Theorem~\ref{theorem:rand_k}), $\zeta_{\cC} = K$ and $\frac{d}{\zeta_{\cC}} = \Theta\left(\omega\right).$}, accordingly, to get $\varepsilon$-solution.

The conclusions are the same as in the general nonconvex case (see Section~\ref{sec:theorems}). 

In the gradient seeting, comparing \eqref{eq:pl_compare:new:gradient} and \eqref{eq:pl_compare:old:gradient}, up to Lipschitz constants factors, we get the degeneration up to $\nicefrac{1}{\probavailable}$ factor due to the partial participation.

In the finite-sum setting, comparing \eqref{eq:pl_compare:new:page} and \eqref{eq:pl_compare:old:page}, we can guarantee the degeneration up to $\nicefrac{1}{\probavailable}$ factor due to the partial participation only if $B = \cO\left(\frac{L_{\max}^2}{\widehat{L}^2}\right)$.

In the stochastic setting, the comparison is a little bit more complicated. As in the finite-sum setting, we have to take 
$B = \cO\left(\frac{L_{\sigma}^2}{\widehat{L}^2}\right)$ to guarantee the degeneration up to $\nicefrac{1}{\probavailable}$ of the term $\frac{\sigma}{\probavailable n \mu^{\nicefrac{3}{2}} \sqrt{\varepsilon B}}\left(\widehat{L} + \frac{L_{\sigma}}{\sqrt{B}}\right)$ from \eqref{eq:pl_compare:old:mvr}. However, \algname{\algorithmname-MVR} has also suboptimal term $\frac{\omega}{\probavailable}\sqrt{\frac{\sigma^2}{\mu n \varepsilon B}}$. This suboptimality is tightly connected with the suboptimality of $B_{\textnormal{init}}$ in the general nonconvex case, which we discuss in Section~\ref{sec:stochastic_setting}, and it also appears in the analysis of \algname{DASHA-MVR} \citep{tyurin2022dasha}.

\alexander{Finish explaining}
% In the following corollary, we consider Rand$K$ compressors (see Definition~\ref{def:rand_k}) and show that with the particular choice of parameters, up to the Lipschitz constants and probability $\probavailable$ factor, \algname{\algorithmname-MVR} gets the optimal oracle complexity and SOTA communication complexity.
Let us provide the analog of Corollary~\ref{cor:stochastic:randk}.
...
% \begin{corollary}
  ...
  % batch size $B \leq \min\left\{\frac{\sigma}{\sqrt{\varepsilon} n}, \frac{L_{\sigma}^2}{\mathbbm{1}_{\probavailable}^2 \widehat{L}^2}\right\},$ 
...
Let us consider the Rand$K$ compressor. In the gradient setting, the communication complexity of \algname{DASHA} equals 
\begin{align*}
  \widetilde{\cO}\left(K \omega + K\frac{L}{\mu} + K \frac{\omega \widehat{L}}{\mu \sqrt{n}}\right) = \widetilde{\cO}\left(d + K\frac{L}{\mu} + \frac{d \widehat{L}}{\mu \sqrt{n}}\right) = \widetilde{\cO}\left(d + \frac{d L}{\mu \sqrt{n}} + \frac{d \widehat{L}}{\mu \sqrt{n}}\right) = \widetilde{\cO}\left(d + \frac{d \widehat{L}}{\mu \sqrt{n}}\right),
\end{align*}
if $K \leq \frac{d}{\sqrt{n}}.$
...
We now provide the analog of Lemma~\ref{lemma:main_lemma}.
