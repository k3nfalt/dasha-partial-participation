...
\begin{table*}[!ht]
  ...
  \emph{PP} (Partial Participation) = Does a method support partial participation from Section~\ref{sec:partial_participation}? \emph{CC} = Does a method have communication complexity equals to $\cO\left(\frac{d}{\sqrt{n} \varepsilon}\right)$ \iffalse w.r.t. the dimension $d$, the number of nodes $n,$ and accuracy $\varepsilon$ \fi ?}
...
$\bullet$ Moreover, this is the first method that combines \textit{variance reduction of stochastic gradients and partial participation} flawlessly: i) it gets the optimal oracle complexity ii) does not require the participation of all nodes iii) does not require the bounded gradients assumption.
