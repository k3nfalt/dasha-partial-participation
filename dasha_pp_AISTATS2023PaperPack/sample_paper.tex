...
\begin{table*}[!ht]
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}
      % \scriptsize
      \item [{\color{blue}(a)}] \algname{MARINA} and \algname{FedPAGE}, with a small probability, require the participation of all nodes so that they can not support partial participation from Section~\ref{sec:partial_participation}. Nevertheless, these methods provide suboptimal oracle complexities.
...
\begin{align*}f_i(x_1, x_2) \eqdef & {\rm E}_{j \sim [m]}\vast[-\log\left(\frac{\exp\left(a_{ij}^\top x_{y_{ij}}\right)}{\sum_{y \in \{1, 2\}} \exp\left(a_{ij}^\top x_{y}\right)}\right) \\
  &+ \lambda \sum_{y \in \{1, 2\}} \sum_{k = 1}^d \frac{\{x_{y}\}_k^2}{1 + \{x_{y}\}_k^2}\vast],\end{align*}
