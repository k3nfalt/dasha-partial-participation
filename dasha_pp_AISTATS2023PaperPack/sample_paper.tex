...
\begin{table*}
  ...
  \begin{threeparttable}
    ...
    \emph{PP} & \emph{CC} & Limitations \\
     ...
     \midrule
     \begin{tabular}{@{}c@{}}\algname{FedAvg} \\ \citep{karimireddy2020scaffold} \end{tabular} & ? & ? & ? & --- \\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{SCAFFOLD} \\ \citep{karimireddy2020scaffold} \end{tabular} & ? & ? & ? & --- \\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{MIME}\textsuperscript{[{\color{blue}(c)}]} \\ \citep{karimireddy2020mime} \end{tabular} & \xmark\textsuperscript{{\color{blue}(d)}} & \cmark & \xmark & \begin{tabular}{@{}c@{}}Calculates full gradient \\ Bounded gradients assumption \end{tabular} \\
     \midrule
...
\item [{\color{blue}(c)}] Note that \algname{MIME} and \algname{DASHA-PP} can be directly compared because \algname{MIME} considers the online version of the problem \eqref{eq:main_problem}, and requires more strict assumptions.
\item [{\color{blue}(d)}] Although \algname{MIME} obtains the convergence rate $\cO\left(\frac{1}{\varepsilon^{3/2}}\right)$ of a variance reduces method, it requires the calculation of the full (exact) gradient.
