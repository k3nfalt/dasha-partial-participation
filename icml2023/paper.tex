...
\icmltitlerunning{A Computation and Communication Efficient Method for Distributed Nonconvex Problems in the Partial Participation Setting}

\begin{document}

\twocolumn[
\icmltitle{A Computation and Communication Efficient Method \\ for Distributed Nonconvex Problems in the Partial Participation Setting}
...
\begin{abstract}
    ...
    % Moreover, we observe that "1 + 1 + 1 is not 3": by mixing variance reduction of stochastic gradients with compressed communication and partial participation, we do not obtain a fully synergetic effect. We explain the nature of this phenomenon, argue that this is to be expected, and propose possible workarounds.
\end{abstract}
...
\begin{table*}[!ht]
  \caption{Summary of methods that solve the problem \eqref{eq:main_problem} in the stochastic setting \eqref{eq:task_staochastic}. Abbr.: \emph{VR} (Variance Reduction) = Does a method have the optimal oracle complexity $\cO\left(\frac{\sigma^2}{\varepsilon} + \frac{\sigma}{\varepsilon^{\nicefrac{3}{2}}}\right)$?
  \emph{PP} (Partial Participation) = Does a method support partial participation from Section~\ref{sec:partial_participation}? \emph{CC} = Does a method have the communication complexity equals to $\cO\left(\frac{d}{\sqrt{n} \varepsilon}\right)$ \iffalse w.r.t. the dimension $d$, the number of nodes $n,$ and accuracy $\varepsilon$ \fi ?}
  ...
  \begin{threeparttable}
    ...
    \emph{PP} & \emph{CC} & Limitations \\
     \midrule\midrule
     \begin{tabular}{@{}c@{}}\algname{SPIDER}, \algname{SARAH}, \algname{PAGE}, \algname{STORM} \\ \citep{SPIDER, SARAH} \\ \citep{PAGE, cutkosky2019momentum}\end{tabular} & \cmark & \xmark & \xmark & ---\\
     \midrule
    %  \begin{tabular}{@{}c@{}} Naive \algname{STORM} in the \emph{PP} setting \end{tabular} & ? & \cmark & \xmark & \\
    %  \midrule
     \begin{tabular}{@{}c@{}}\algname{MARINA} \\ \citep{MARINA} \end{tabular} & \cmark & \xmark\textsuperscript{{\color{blue}(a)}} & \cmark\textsuperscript{{\color{blue}(b)}} & \begin{tabular}{@{}c@{}}Suboptimal convergence rate \\ (see \citep{tyurin2022dasha}). \end{tabular}\\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{FedPAGE} \\ \citep{zhao2021fedpage} \end{tabular} & \xmark & \xmark\textsuperscript{{\color{blue}(a)}} & \xmark & Suboptimal oracle complexity $\cO\left(\frac{\sigma^2}{\varepsilon^2}\right)$. \\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{FRECON} \\ \citep{zhao2021faster} \end{tabular} & \xmark & \cmark & \cmark & --- \\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{FedAvg} \\ \citep{mcmahan2017communication, karimireddy2020scaffold} \end{tabular} & \xmark & \cmark & \xmark & Bounded gradients (dissimilarity) assumption of $f_i$.\\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{SCAFFOLD} \\ \citep{karimireddy2020scaffold} \end{tabular} & \xmark & \cmark & \xmark & Suboptimal convergence rate\textsuperscript{{\color{blue}(e)}}. \\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{MIME}\textsuperscript{{\color{blue}(c)}} \\ \citep{karimireddy2020mime} \end{tabular} & \xmark\textsuperscript{{\color{blue}(d)}} & \cmark & \xmark & \begin{tabular}{@{}c@{}}Calculates full gradient. \\ Bounded gradients (dissimilarity) assumption of $f_i$. \\ Suboptimal oracle complexity $\cO\left(\nicefrac{1}{\varepsilon^{3/2}}\right)$ in the setting \eqref{eq:task_minibatch}.\end{tabular} \\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{CE-LSGD} (for Partial Participation)\textsuperscript{{\color{blue}(c)}} \\ \citep{pateltowards} \\ (concurrent work) \end{tabular} & \cmark & \cmark & \xmark & \begin{tabular}{@{}c@{}}Bounded gradients (dissimilarity) assumption of $f_i$. \\ Suboptimal oracle complexity $\cO\left(\nicefrac{1}{\varepsilon^{3/2}}\right)$ in the setting \eqref{eq:task_minibatch}.\end{tabular} \\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{DASHA} \\ \citep{tyurin2022dasha} \end{tabular} & \begin{tabular}{@{}c@{}}\cmark \\ \phantom{or} \\ \xmark \end{tabular} & \begin{tabular}{@{}c@{}}\xmark \\ or\\ \cmark \end{tabular} & \begin{tabular}{@{}c@{}}\cmark \\ \phantom{or}\\ \cmark \end{tabular} & --- \\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{DASHA-PP} \\ (new) \end{tabular} & \cmark & \cmark & \cmark & --- \\
    \midrule\midrule
    \end{tabular}
    \begin{tablenotes}
      % \scriptsize
      \item [{\color{blue}(a)}] \algname{MARINA} and \algname{FedPAGE}, with a small probability, require the participation of all nodes so that they can not support partial participation from Section~\ref{sec:partial_participation}. Moreover, these methods provide suboptimal oracle complexities.
      \item [{\color{blue}(b)}] On average, \algname{MARINA} provides the compressed communication mechanism with complexity $\cO\left(\frac{d}{\sqrt{n} \varepsilon}\right).$ However, with a small probability, this method sends non-compressed vectors.
      \item [{\color{blue}(c)}] Note that \algname{MIME} and \algname{CE-LSGD} can not be directly compared with \algname{DASHA-PP} because \algname{MIME} and \algname{CE-LSGD} consider the online version of the problem \eqref{eq:main_problem}, and require more strict assumptions.
      \item [{\color{blue}(d)}] Although \algname{MIME} obtains the convergence rate $\cO\left(\frac{1}{\varepsilon^{3/2}}\right)$ of a variance reduced method, it requires the calculation of the full (exact) gradients.
      \item [{\color{blue}(e)}] It can be seen when $\sigma^2 = 0.$ Let us consider the $s$-nice sampling of the nodes, then \algname{SCAFFOLD} requires $\cO\left(\frac{n^{3/2}}{\varepsilon s^{3/2}}\right)$ communication rounds to get $\varepsilon$-solution, while \algname{DASHA-PP} requires $\cO\left(\frac{\sqrt{n}}{\varepsilon s}\right)$ communication rounds (see Theorem~\ref{theorem:stochastic} with $\omega = 0,$ $b = \frac{\probavailable}{2 - \probavailable},$ and $\probavailable = \frac{s}{n}$).
...
\begin{table*}[!ht]
  ...
  \begin{threeparttable}
    ...
    \emph{PP} & \emph{CC} & Limitations \\
     \midrule\midrule
     \begin{tabular}{@{}c@{}}\algname{SPIDER}, \algname{PAGE} \\ \citep{SPIDER, PAGE} \end{tabular} & \cmark & \xmark & \xmark & ---\\
     \midrule
    %  \begin{tabular}{@{}c@{}} Naive \algname{STORM} in the \emph{PP} setting \end{tabular} & ? & \cmark & \xmark & \\
    %  \midrule
     \begin{tabular}{@{}c@{}}\algname{MARINA} \\ \citep{MARINA} \end{tabular} & \cmark & \xmark\textsuperscript{{\color{blue}(a)}} & \cmark\textsuperscript{{\color{blue}(b)}} & \begin{tabular}{@{}c@{}}Suboptimal convergence rate \\ (see \citep{tyurin2022dasha}). \end{tabular}\\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{ZeroSARAH} \\ \citep{li2021zerosarah} \end{tabular} & \cmark & \cmark & \xmark & Only homogeneous regime, i.e., the functions $f_i$ are equal. \\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{FedPAGE} \\ \citep{zhao2021fedpage} \end{tabular} & \xmark & \xmark\textsuperscript{{\color{blue}(a)}} & \xmark & Suboptimal oracle complexity $\cO\left(\frac{m}{\varepsilon}\right)$. \\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{DASHA} \\ \citep{tyurin2022dasha} \end{tabular} & \cmark & \xmark & \cmark & --- \\
     \midrule
     \begin{tabular}{@{}c@{}}\algname{DASHA-PP} \\ (new) \end{tabular} & \cmark & \cmark & \cmark & --- \\
    \midrule\midrule
    \end{tabular}
...
\section{Algorithm Description and Main Challenges To Partial Participation}
...
$\bullet$ In fact, \algname{DASHA} supports the partial participation of nodes \emph{in the gradient setting}. Since the nodes only do the following steps (see full algorithm in Algorithm~\ref{alg:main_algorithm_dasha}):
\begin{align*}
...
\begin{align*}
  \cC_{i,p} \eqdef \begin{cases}
    \frac{1}{p}\cC_{i},& \textnormal{with probability } p\\
    0,& \textnormal{with probability } 1 - p\\
  \end{cases}
\end{align*}
With probability $1 - p,$ a node does not update $g_i^{\cdot}$ and does not send anything to the server. The main observation is that we can do this trick since the nodes only need the points $x^{t+1}$ and $x^{t}$ to do the step.
...
$\bullet$ However, we focus our attention on partial participation in the finite-sum and stochastic settings. Let us consider the nodes' steps in \algname{DASHA-MVR} (see Algorithm~\ref{alg:main_algorithm_dasha_mvr}):
\begin{align*}
  &h^{t+1}_i = \nabla f_i(x^{t+1};\xi^{t+1}_{i}) + (1 - b) (h^t_i - \nabla f_i(x^{t};\xi^{t+1}_{i})) \\
  &g^{t+1}_i = g^{t}_i + \cC_i\left(h^{t+1}_i - h^{t}_i - a \left(g^{t}_i - h^{t}_i\right)\right)
\end{align*}
...
At the first reading of the proofs, we suggest the reader follow the proof of Theorem~\ref{theorem:gradient_oracle} in the gradient setting, which takes a small part of the paper. Although the proof seems to be dense and large, the size of the appendix is justified by the fact that we consider different settings and P\L-condition.
...
In the following corollary, we consider Rand$K$ compressors (see Definition~\ref{def:rand_k}) and show that with the particular choice of parameters, up to the Lipschitz constants factors, \algname{\algorithmname-MVR} gets the optimal oracle complexity and SOTA communication complexity of \algname{DASHA-SYNC-MVR} method.
