...
In Figure~\ref{fig:stochastic}, we consider the stochastic setting. We can see that \algname{DASHA-PP} has better convergence rates than \algname{FRECON}. \algname{DASHA-PP} with $s \in \{5, 10\}$ even improves \algname{MARINA} that works in the \emph{full participation regime} (one should expect that convergence rate of \algname{MARINA} with partial participation will be even worse).
