...
\begin{align*}
  \cC_{i}^p \eqdef \begin{cases}
    \frac{1}{p}\cC_{i},& \textnormal{with } \probavailable,\\
    0,& \textnormal{with } 1 - \probavailable.
...
$\bullet$ However, we focus our attention on partial participation \emph{in the finite-sum and stochastic settings}. Consider the nodes' steps in \algname{DASHA-MVR} (see Algorithm~\ref{alg:main_algorithm_dasha_mvr}) that is designed for the stochastic setting:
\begin{align*}
