...
\begin{lemma}
  ...
  \begin{align}
      ...
      \leq \frac{2 \omega}{\probavailable n^2}\sum_{i=1}^n\norm{k^{t+1}_i}^2 + \frac{a^2 (2\omega + 1 - p)}{\probavailable n^2}\sum_{i=1}^n\norm{g^{t}_i - h^{t}_i}^2 + (1 - a)^2\norm{g^{t} - h^{t}}^2,
  \end{align}
  and
  \begin{align}
      \label{eq:compressor_local_error}
      \ExpSub{\cC}{\ExpSub{\probavailable}{\norm{g^{t+1}_i - h^{t+1}_i}^2}} \leq \frac{2 \omega}{\probavailable}\norm{k^{t+1}_i}^2 + \frac{a^2 (2\omega + 1 - p)}{\probavailable}\norm{g^{t}_i - h^{t}_i}^2 + (1 - a)^2\norm{g^{t}_i - h^{t}_i}^2, \quad \forall i \in [n].
  \end{align}
\end{lemma}
\begin{proof}
