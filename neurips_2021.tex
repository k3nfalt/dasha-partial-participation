...
\begin{proof}
  ...
  \end{align*}
  Using \eqref{sec:auxiliary_facts_partial_participation:one}, \eqref{sec:auxiliary_facts_partial_participation:two}, and \eqref{auxiliary:variance_decomposition}, we get
  \begin{align*}
    &\ExpSub{B}{\ExpSub{\probavailable}{\norm{h^{t+1}_{2} - \ExpSub{B}{\ExpSub{\probavailable}{h^{t+1}_{2}}}}^2}}\\
    ...
    &\quad + \frac{\probavailable - \probpairaa}{n^2 \probavailable^2}\sum_{i=1}^n\norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t})}^2.
  \end{align*}
  In the last inequality, we used \eqref{eq:partial_participation_constraint}.
\end{proof}
