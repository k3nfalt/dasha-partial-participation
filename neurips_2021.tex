...
\alexander{
  TODO: 
  1) Rewrite the proof of DASHA-PP-PAGE
  2) Add Hessian Varaince
  3) In general, refactor proofs for simplicity
}
...
\begin{table*}
  ...
  \begin{threeparttable}
    \begin{tabular}{cccccc}
      \midrule
    Setting & Method & Communication Rounds\tnote{\color{blue}(a)}& 
    Oracle Complexity & Sync?\tnote{\color{blue}(b)} \\
     \midrule\midrule
     \multirow{2}{*}{\tablesmall{Gradient}} 
     & \algnamesmall{PP-MARINA} &   $\frac{1}{\varepsilon} + \frac{(\omega + 1)\sqrt{n}}{\varepsilon s}$ & $T$ & Yes \\
     & \algnamesmall{FRECON} &   $\frac{1}{\varepsilon} + \frac{(\omega + 1)\sqrt{n}}{\varepsilon s}$ & $T$ & Yes \\
     & \algnamesmall{DASHA-PP} (New) \tablescriptsize{(Cor.~\ref{cor:gradient_oracle})} &  $\frac{1}{\varepsilon} + \frac{(\omega + 1)\sqrt{n}}{\varepsilon s}$ & $T$ & \improvement{No} \\
      \cmidrule(l  r ){1-5}
      \multirow{2}{*}{\tablesmall{\begin{tabular}{c}Finite-Sum\\ \eqref{eq:task_minibatch}\end{tabular}}} 
      & \algnamesmall{DASHA-PP-PAGE} \tablescriptsize{(Cor.~\ref{cor:mini_batch_oracle})} &  $\frac{1 + \nicefrac{\omega}{\sqrt{n}}}{\varepsilon} + \improvement{\frac{\sqrt{m}}{\varepsilon \sqrt{n} B}}$ & $m + B T$ & \improvement{No} \\
    \cmidrule(l  r ){1-5}
    \multirow{3}{*}{\tablesmall{\begin{tabular}{c}Stochastic\\ \eqref{eq:task_staochastic}\end{tabular}}} & \algnamesmall{VR-MARINA (online)} &   $\frac{1 + \nicefrac{\omega}{\sqrt{n}}}{\varepsilon} + \frac{\sigma^2}{\varepsilon n B} + \frac{\sqrt{1 + \omega}\sigma}{\varepsilon^{\nicefrac{3}{2}} n B}$ & $B\omega + BT$ & Yes \\
                                & \algnamesmall{DASHA-MVR} \tablescriptsize{(Cor.~\ref{cor:stochastic})} &  $\frac{1 + \nicefrac{\omega}{\sqrt{n}}}{\varepsilon} + \frac{\sigma^2}{\varepsilon n B} + \improvement{\frac{\sigma}{\varepsilon^{\nicefrac{3}{2}} n B}}$ & $\degeneration{B\omega\sqrt{\frac{\sigma^2}{\varepsilon n B}}} + BT$ & \improvement{No} \\
                                & \algnamesmall{DASHA-SYNC-MVR} \tablescriptsize{(Cor.~\ref{cor:sync_stochastic})} &  $\frac{1 + \nicefrac{\omega}{\sqrt{n}}}{\varepsilon} + \frac{\sigma^2}{\varepsilon n B} + \improvement{\frac{\sigma}{\varepsilon^{\nicefrac{3}{2}} n B}}$ & $B\omega + BT$ & Yes \\
    \midrule\midrule
    \end{tabular}
\begin{tablenotes}
% \scriptsize
\item [{\color{blue}(a)}] Only dependencies w.r.t.\,the following variables are shown: $\omega = $ quantization parameter, $n = \#$ of nodes, $m = \#$ of local functions (only in finite-sum case \eqref{eq:task_minibatch}), $\sigma^2 = $ variance of stochastic gradients (only in stochastic case \eqref{eq:task_staochastic}), $B = $ batch size (only in finite-sum and stochastic case). To simplify bounds, we assume that $\omega + 1 = \Theta\left(\nicefrac{d}{\zeta_{\cC}}\right),$ where $d$ is dimension of $x$ in \eqref{eq:main_problem} and $\zeta_{\cC}$ is the expected number of nonzero coordinates that each compressor $\cC_i$ returns (see Definition~\ref{def:expected_density}).
...
\begin{algorithm}[H]
  ...
  probability $\probpage \in (0, 1]$ (only in \algname{DASHA}-\algname{PP}-\algname{PAGE}),
  ...
  \IF{$i^{\textnormal{th}} \textnormal{ node is \textit{participating}}$\textsuperscript{\red (a)}}
      ...
      \begin{cases}
          ...
          \end{cases}  \\
          \begin{cases}
            \nabla f_i(x^{t+1}) - h^t_i,& \textnormal{with probability $\probpage$ on all \textit{participating}\textsuperscript{\red (a)} nodes} \\
            \frac{1}{B}\sum_{j \in I^t_i}\left(\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t})\right), & \textnormal{with probability $1 - \probpage$ on all \textit{participating}\textsuperscript{\red (a)} nodes} 
          \end{cases}  \\
          \vspace{0.15cm}
          ...
          \begin{cases}
            \frac{1}{B'}\sum_{k=1}^{B'} \nabla f_i(x^{t+1};\xi^{t+1}_{k}) - h^t_i,& \\
            \textnormal{with probability $\probmega$ on all \textit{participating}\textsuperscript{\red (a)} nodes} \\
            \frac{1}{B}\sum_{j=1}^B \nabla f_i(x^{t+1};\xi^{t+1}_{ij}) - \frac{1}{B}\sum_{j=1}^B \nabla f_i(x^{t};\xi^{t+1}_{ij}), & \\
            \textnormal{with probability $1 - \probmega$ on all \textit{participating}\textsuperscript{\red (a)} nodes} 
        \end{cases}  \\
      \end{cases}$
...
\subsection{Proof for \algname{DASHA}-\algname{PP}}

\begin{lemma}
  \label{lemma:gradient}
  Suppose that Assumptions~\ref{ass:nodes_lipschitz_constant} and \ref{ass:partial_participation} hold. For $h^{t+1}_i$ and $k^{t+1}_i$ from Algorithm~\ref{alg:main_algorithm} (\algname{DASHA}-\algname{PP}) we have
  \begin{enumerate}
  \item
      \begin{align*}
...
\subsection{Proof for \algname{DASHA}-\algname{PP}-\algname{PAGE}}
...
\begin{lemma}
  ...
  Suppose that Assumptions \ref{ass:nodes_lipschitz_constant}, \ref{ass:max_lipschitz_constant}, and \ref{ass:partial_participation} hold. For $h^{t+1}_i$ and $k^{t+1}_i$ from Algorithm~\ref{alg:main_algorithm} (\algname{DASHA}-\algname{PP}-\algname{PAGE}) we have
  \begin{enumerate}
  \item
      \begin{align*}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\quad +(1 - b)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &=\probavailable \ExpSub{B}{\ExpSub{\probpage}{\norm{h^{t}_i + \frac{1}{\probavailable}k^{t+1}_i - \left(\nabla f_i(x^{t+1}) - (1 - b)( h^t_i - \nabla f_i(x^{t}))\right)}^2}} \\
    &\quad + (1 - \probavailable) \norm{h^{t}_i - \left(\nabla f_i(x^{t+1}) - (1 - b)( h^t_i - \nabla f_i(x^{t}))\right)}^2 \\
    &\quad +(1 - b)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &=\probavailable \ExpSub{B}{\ExpSub{\probpage}{\norm{h^{t}_i + \frac{1}{\probavailable}k^{t+1}_i - \left(\nabla f_i(x^{t+1}) - (1 - b)( h^t_i - \nabla f_i(x^{t}))\right)}^2}} \\
    &\quad + (1 - \probavailable) \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - b)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &=\frac{1}{\probavailable} \ExpSub{B}{\ExpSub{\probpage}{\norm{k^{t+1}_i - \ExpSub{B}{\ExpSub{\probpage}{k^{t+1}_i}}}^2}} \\
    &\quad + \frac{(1 - \probavailable)}{\probavailable} \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - b)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &=\frac{\probpage}{\probavailable} \ExpSub{B}{\norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \frac{b}{\probpage} \left(h^t_i - \nabla f_i(x^{t})\right) - \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))\right)}^2} \\
    &\quad + \frac{1 - \probpage}{\probavailable} \ExpSub{B}{\norm{\frac{1}{B}\sum_{j \in I^t_i}\left(\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t})\right) - \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))\right)}^2} \\
    &\quad + \frac{(1 - \probavailable)}{\probavailable} \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - b)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &=\frac{b^2 (1 - \probpage)^2}{\probavailable \probpage} \norm{h^t_i - \nabla f_i(x^{t})}^2 \\
    &\quad + \frac{1 - \probpage}{\probavailable} \ExpSub{B}{\norm{\frac{1}{B}\sum_{j \in I^t_i}\left(\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t})\right) - \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))\right)}^2} \\
    &\quad + \frac{(1 - \probavailable)}{\probavailable} \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - b)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &=\frac{b^2 (1 - \probpage)}{\probavailable \probpage} \norm{h^t_i - \nabla f_i(x^{t})}^2 \\
    &\quad + \frac{1 - \probpage}{\probavailable} \ExpSub{B}{\norm{\frac{1}{B}\sum_{j \in I^t_i}\left(\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t})\right) - \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t})\right)}^2} \\
    &\quad + \frac{(1 - \probavailable)}{\probavailable} \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - b)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
  \end{align*}

  \alexander{New approach with $1 - \probpage$:}
  \begin{align*}
    &\ExpSub{B}{\ExpSub{\probavailable}{\ExpSub{\probpage}{\norm{h^{t+1}_i - \nabla f_i(x^{t+1})}^2}}} \\
    &=\ExpSub{B}{\ExpSub{\probavailable}{\ExpSub{\probpage}{\norm{h^{t+1}_i - \left(\nabla f_i(x^{t+1}) - (1 - \probpage)( h^t_i - \nabla f_i(x^{t}))\right)}^2}}} \\
    &\quad +(1 - \probpage)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &=\probavailable \ExpSub{B}{\ExpSub{\probpage}{\norm{h^{t}_i + \frac{1}{\probavailable}k^{t+1}_i - \left(\nabla f_i(x^{t+1}) - (1 - \probpage)( h^t_i - \nabla f_i(x^{t}))\right)}^2}} \\
    &\quad + (1 - \probavailable) \norm{h^{t}_i - \left(\nabla f_i(x^{t+1}) - (1 - \probpage)( h^t_i - \nabla f_i(x^{t}))\right)}^2 \\
    &\quad +(1 - \probpage)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &=\probavailable \ExpSub{B}{\ExpSub{\probpage}{\norm{h^{t}_i + \frac{1}{\probavailable}k^{t+1}_i - \left(\nabla f_i(x^{t+1}) - (1 - \probpage)( h^t_i - \nabla f_i(x^{t}))\right)}^2}} \\
    &\quad + (1 - \probavailable) \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \probpage(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - \probpage)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &=\frac{1}{\probavailable} \ExpSub{B}{\ExpSub{\probpage}{\norm{k^{t+1}_i - \ExpSub{B}{\ExpSub{\probpage}{k^{t+1}_i}}}^2}} \\
    &\quad + \frac{(1 - \probavailable)}{\probavailable} \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \probpage(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - \probpage)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &=\frac{\probpage}{\probavailable} \ExpSub{B}{\norm{\nabla f_i(x^{t+1}) - h^t_i - \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \probpage(h^t_i - \nabla f_i(x^{t}))\right)}^2} \\
    &\quad + \frac{1 - \probpage}{\probavailable} \ExpSub{B}{\norm{\frac{1}{B}\sum_{j \in I^t_i}\left(\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t})\right) - \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \probpage(h^t_i - \nabla f_i(x^{t}))\right)}^2} \\
    &\quad + \frac{(1 - \probavailable)}{\probavailable} \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \probpage(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - \probpage)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &=\frac{\probpage (1 - \probpage)^2}{\probavailable} \ExpSub{B}{\norm{h^t_i - \nabla f_i(x^{t})}^2} \\
    &\quad + \frac{1 - \probpage}{\probavailable} \ExpSub{B}{\norm{\frac{1}{B}\sum_{j \in I^t_i}\left(\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t})\right) - \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \probpage(h^t_i - \nabla f_i(x^{t}))\right)}^2} \\
    &\quad + \frac{(1 - \probavailable)}{\probavailable} \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \probpage(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - \probpage)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &\leq \frac{\probpage (1 - \probpage)^2}{\probavailable} \ExpSub{B}{\norm{h^t_i - \nabla f_i(x^{t})}^2} \\
    &\quad + \frac{2(1 - \probpage)}{\probavailable} \ExpSub{B}{\norm{\frac{1}{B}\sum_{j \in I^t_i}\left(\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t})\right) - \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t})\right)}^2} \\
    &\quad + \frac{2(1 - \probpage)\probpage^2}{\probavailable} \norm{h^t_i - \nabla f_i(x^{t})}^2 \\
    &\quad + \frac{(1 - \probavailable)}{\probavailable} \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \probpage(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - \probpage)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
  \end{align*}
  ...
  \begin{align*}
    ...
    &\quad + \frac{(1 - \probavailable)}{\probavailable} \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - b)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &=\frac{\probpage}{\probavailable} \ExpSub{B}{\norm{\left(\frac{1}{\probpage} - \left(\frac{1}{\probpage} - 1\right)\beta_1\right) \nabla f_i(x^{t+1}) + \left(- \frac{1}{\probpage} \left(1 - b\right) + \left(\frac{1}{\probpage} - 1\right) \beta_1\right) \nabla f_i(x^{t}) + \left(-\frac{b}{\probpage} - \left(\frac{1}{\probpage} - 1\right) \beta_3 h^t_i\right)}^2} \\
    &\quad + \frac{1 - \probpage}{\probavailable} \ExpSub{B}{\norm{\beta_1 \frac{1}{B}\sum_{j \in I^t_i}\left(\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t})\right) + \beta_3 h^t_i - (\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t})))}^2} \\
    &\quad + \frac{(1 - \probavailable)}{\probavailable} \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - b)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
  \end{align*}
  ...
  \begin{align*}
    ...
    &\quad + \frac{(1 - \probavailable)}{\probavailable} \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - b)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
    &=\frac{\probmega}{\probavailable} \ExpSub{B}{\norm{\nabla f_i(x^{t+1};\xi^{t+1}_{k}) - h^t_i - \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \probmega(h^t_i - \nabla f_i(x^{t}))\right)}^2} \\
    &\quad \frac{1 - \probmega}{\probavailable} \ExpSub{B}{\ExpSub{\probpage}{\norm{\nabla f_i(x^{t+1};\xi^{t+1}_{ij}) - \nabla f_i(x^{t};\xi^{t+1}_{ij})- \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \probmega(h^t_i - \nabla f_i(x^{t}))\right)}^2}} \\
    &\quad + \frac{(1 - \probavailable)}{\probavailable} \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \probmega(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - \probmega)^2\norm{h^{t}_i - \nabla f_i(x^{t})}^2 \\
  \end{align*}

%   \begin{cases}
%     \frac{1}{B'}\sum_{k=1}^{B'} \nabla f_i(x^{t+1};\xi^{t+1}_{k}) - \frac{1}{B'}\sum_{k=1}^{B'} \nabla f_i(x^{t};\xi^{t+1}_{k}) - \frac{b}{\probmega}\left(h^t_i - \frac{1}{B'}\sum_{k=1}^{B'} \nabla f_i(x^{t};\xi^{t+1}_{k})\right),& \\
%     \textnormal{with probability $\probmega$ on all \textit{participating}\textsuperscript{\red (a)} nodes} \\
%     \frac{1}{B}\sum_{j=1}^B \nabla f_i(x^{t+1};\xi^{t+1}_{ij}) - \frac{1}{B}\sum_{j=1}^B \nabla f_i(x^{t};\xi^{t+1}_{ij}), & \\
%     \textnormal{with probability $1 - \probmega$ on all \textit{participating}\textsuperscript{\red (a)} nodes} 
% \end{cases}  \\
...
\subsection{Proof for \algname{DASHA-PP-FINITE-MEMORY}}
...
\begin{proof}
  \begin{align*}
    &\ExpSub{B}{\ExpSub{\probavailable}{\norm{h^{t+1} - \nabla f(x^{t+1})}^2}} \\
    &=\ExpSub{B}{\ExpSub{\probavailable}{\norm{h^{t+1} - \left(\nabla f(x^{t+1}) + (1 - b)(h^t - \nabla f(x^{t}))\right)}^2}} \\
    &\quad +(1 - b)^2\norm{h^{t} - \nabla f(x^{t})}^2 \\
    &=\ExpSub{B}{\ExpSub{\probavailable}{\norm{h^{t} + \frac{1}{n \probavailable} \sum_{i \in S} \frac{1}{B} \sum_{j \in I^t_i} \left(\left(\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t}) - b \left(h^t_{ij} - \nabla f_{ij}(x^{t})\right)\right)\right) - \left(\nabla f(x^{t+1}) + (1 - b)(h^t - \nabla f(x^{t}))\right)}^2}} \\
    &\quad +(1 - b)^2\norm{h^{t} - \nabla f(x^{t})}^2 \\  
    &\leq 2\ExpSub{B}{\ExpSub{\probavailable}{\norm{\frac{1}{n \probavailable} \sum_{i \in S} \frac{1}{B} \sum_{j \in I^t_i} \left(\left(\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t}) - b \left(h^t_{ij} - \nabla f_{ij}(x^{t})\right)\right)\right) - \left(\nabla f(x^{t+1}) - b(h^t - \nabla f(x^{t}))\right)}^2}} \\
    &\quad + 2\norm{\nabla f(x^{t+1}) - \nabla f(x^{t})}^2 \\
    &\quad +(1 - b)^2\norm{h^{t} - \nabla f(x^{t})}^2 \\  
    &= 2\ExpSub{B}{\ExpSub{\probavailable}{\norm{\frac{1}{s} \sum_{i \in S} \frac{1}{B} \sum_{j \in I^t_i} \left(\left(\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t}) - b \left(h^t_{ij} - \nabla f_{ij}(x^{t})\right)\right)\right) - \left(\nabla f(x^{t+1}) - b(h^t - \nabla f(x^{t}))\right)}^2}} \\
    &\quad + 2\norm{\nabla f(x^{t+1}) - \nabla f(x^{t})}^2 \\
    &\quad +(1 - b)^2\norm{h^{t} - \nabla f(x^{t})}^2 \\  
  \end{align*}
  \alexander{Independent, for simplicity:}
  \begin{align*}
    &\ExpSub{B}{\ExpSub{\probavailable}{\norm{h^{t+1} - \nabla f(x^{t+1})}^2}} \\
    &=\ExpSub{B}{\ExpSub{\probavailable}{\norm{h^{t+1} - \left(\nabla f(x^{t+1}) + (1 - b)(h^t - \nabla f(x^{t}))\right)}^2}} \\
    &\quad +(1 - b)^2\norm{h^{t} - \nabla f(x^{t})}^2 \\
    ...
    &\quad +\frac{1 - \probavailable}{n^2} \sum_{i=1}^n \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - b)^2\norm{h^{t} - \nabla f(x^{t})}^2 \\
    &=\frac{1}{\probavailable n^2} \sum_{i=1}^n \ExpSub{B}{\norm{\frac{1}{B}\sum_{j \in I^t_i}\left(\left(\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t}) - b \left(h^t_{ij} - \nabla f_{ij}(x^{t})\right)\right)\right) - \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))\right)}^2} \\
    &\quad +\frac{1 - \probavailable}{n^2 \probavailable} \sum_{i=1}^n \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - b)^2\norm{h^{t} - \nabla f(x^{t})}^2 \\
    &=\frac{1}{\probavailable B n^2 m} \sum_{i=1}^n \sum_{i=1}^m \norm{\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t}) - b \left(h^t_{ij} - \nabla f_{ij}(x^{t})\right) - \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))\right)}^2 \\
    &\quad +\frac{1 - \probavailable}{n^2 \probavailable} \sum_{i=1}^n \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - b)^2\norm{h^{t} - \nabla f(x^{t})}^2 \\
    &=\frac{1}{\probavailable B n^2 m} \sum_{i=1}^n \sum_{i=1}^m \norm{\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t}) - b \left(h^t_{ij} - \nabla f_{ij}(x^{t})\right)}^2 \\
    &\quad +\left(\frac{1 - \probavailable}{n^2 \probavailable} - \frac{1}{\probavailable B n^2}\right) \sum_{i=1}^n \norm{\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))}^2 \\
    &\quad +(1 - b)^2\norm{h^{t} - \nabla f(x^{t})}^2 \\
  \end{align*}
...
\subsection{Proof for \algname{DASHA}-\algname{PP}-\algname{MVR}}

Let us denote $\nabla f_i(x^{t+1};\xi^{t+1}_i) \eqdef \frac{1}{B} \sum_{j=1}^B \nabla f_i(x^{t+1};\xi^{t+1}_{ij}).$

\begin{lemma}
  Suppose that Assumptions \ref{ass:nodes_lipschitz_constant}, \ref{ass:stochastic_unbiased_and_variance_bounded}, \ref{ass:mean_square_smoothness} and \ref{ass:partial_participation} hold. For $h^{t+1}_i$ and $k^{t+1}_i$ from Algorithm~\ref{alg:main_algorithm} (\algname{DASHA}-\algname{PP}-\algname{MVR}) we have
  \begin{enumerate}
  \item
      \begin{align*}
          &\ExpSub{k}{\ExpSub{\probavailable}{\norm{h^{t+1} - \nabla f(x^{t+1})}^2}} \\
