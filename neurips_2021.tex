...
\begin{proof}
  ...
  \begin{align*}
      ...
      &h^{t+1}_{i,1} \eqdef \begin{cases}
          h^t_i + \frac{1}{p} k^{t+1}_{i, 1},& \textnormal{with probability $\probavailable$,} \\
          h^t_i, & \textnormal{with probability $1 - \probavailable$,} 
      \end{cases}  \\
      &h^{t+1}_{i,2} \eqdef \begin{cases}
          h^t_i + \frac{1}{p} k^{t+1}_{i, 2},& \textnormal{with probability $\probavailable$,} \\
          h^t_i, & \textnormal{with probability $1 - \probavailable$,}
      \end{cases}  \\
  \end{align*}
  $h^{t+1}_{1} \eqdef \frac{1}{n}\sum_{i=1}^n h^{t+1}_{i,1},$ and $h^{t+1}_{2} \eqdef \frac{1}{n}\sum_{i=1}^n h^{t+1}_{i,2}.$
\end{proof}
