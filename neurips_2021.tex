...
There are three different sources of randomness in Algorithm~\ref{alg:main_algorithm}: the first one from vectors $\{h_i^{t+1}\}_{i=1}^n$, the second one from compressors $\{\cC_i\}_{i=1}^n$, and the third one from availability of nodes. In this section, we define $\ExpSub{h}{\cdot}$, $\ExpSub{\cC}{\cdot}$ and $\ExpSub{\probavailable}{\cdot}$ to be conditional expectations w.r.t.\,$\{h_i^{t+1}\}_{i=1}^n$, $\{\cC_i\}_{i=1}^n, $ and availability, accordingly, conditioned on all previous randomness. Moreover, we define $\ExpSub{t+1}{\cdot}$ to be a conditional expectation w.r.t. all randomness in iteration $t+1$ conditioned on all previous randomness. Note, that $\ExpSub{t+1}{\cdot}{\cdot} = \ExpSub{h}{\ExpSub{\cC}{\ExpSub{\probavailable}{\cdot}}}.$
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\quad + (1 - a)^2\norm{g^{t} - h^{t}}^2.\\
  \end{align*}
  \alexander{TODO: second inequality}
\end{proof}
...
\begin{proof}
  First, let us proof the bound for $\ExpSub{h}{\ExpSub{\probavailable}{\norm{h^{t+1} - \nabla f(x^{t+1})}^2}}$:
  \begin{align*}
      &\ExpSub{h}{\ExpSub{\probavailable}{\norm{h^{t+1} - \nabla f(x^{t+1})}^2}} \\
      &=\ExpSub{h}{\ExpSub{\probavailable}{\norm{h^{t+1} - \ExpSub{h}{\ExpSub{\probavailable}{h^{t+1}}}}^2}} + \norm{\ExpSub{h}{\ExpSub{\probavailable}{h^{t+1}}} - \nabla f(x^{t+1})}^2.
  \end{align*}
  Using
  \begin{align*}
      \ExpSub{h}{\ExpSub{\probavailable}{h^{t+1}_i}} = h^{t}_i + \ExpSub{h}{k^{t+1}_i} = h^{t}_i + \nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - b(h^t_i - \nabla f_i(x^{t}))
  \end{align*}
  and Assumption~\ref{ass:prob_sampling}, we have
  \begin{align*}
      &\ExpSub{h}{\ExpSub{\probavailable}{\norm{h^{t+1} - \nabla f(x^{t+1})}^2}} \\
      &=\ExpSub{h}{\ExpSub{\probavailable}{\norm{h^{t+1} - \ExpSub{h}{\ExpSub{\probavailable}{h^{t+1}}}}^2}} + \left(1 - b\right)^2 \norm{h^{t} - \nabla f(x^{t})}^2\\
      &=\frac{1}{n^2}\sum_{i=1}^n \ExpSub{h}{\ExpSub{\probavailable}{\norm{h^{t+1}_i - \ExpSub{h}{\ExpSub{\probavailable}{h^{t+1}_i}}}^2}} \\
      &\quad+ \frac{1}{n^2}\sum_{i\neq j}^n \ExpSub{h}{\ExpSub{\probavailable}{\inp{h^{t+1}_i - \ExpSub{h}{\ExpSub{\probavailable}{h^{t+1}_i}}}{h^{t+1}_j - \ExpSub{h}{\ExpSub{\probavailable}{h^{t+1}_j}}}}} \\
      &\quad + \left(1 - b\right)^2 \norm{h^{t} - \nabla f(x^{t})}^2 \\
      &=\frac{\probavailable}{n^2}\sum_{i=1}^n \ExpSub{h}{\norm{h^{t}_i + \frac{1}{\probavailable}k^{t+1}_i - h^{t}_i - \nabla f_i(x^{t+1}) + \nabla f_i(x^{t}) + b(h^t_i - \nabla f_i(x^{t}))}^2} \\
      ...
      &\quad + \left(1 - b\right)^2 \norm{h^{t} - \nabla f(x^{t})}^2.
  \end{align*}
  Next, from the independence of stochastic gradients, we obtain
  \begin{align*}
    &\ExpSub{h}{\ExpSub{\probavailable}{\norm{h^{t+1} - \nabla f(x^{t+1})}^2}} \\
    ...
    &\qquad\qquad\qquad\qquad \nabla f_j(x^{t+1}) - \nabla f_j(x^{t}) - b(h^t_j - \nabla f_j(x^{t}))\Bigg\rangle \\
    &\quad + \left(1 - b\right)^2 \norm{h^{t} - \nabla f(x^{t})}^2 \\
    &\overset{\eqref{auxiliary:variance_decomposition}}{=}\frac{1}{n^2 \probavailable}\sum_{i=1}^n \ExpSub{h}{\norm{k^{t+1}_i - \ExpSub{h}{k^{t+1}_i}}^2} \\
  ...
  \begin{align*}
    &\ExpSub{h}{\ExpSub{\probavailable}{\norm{h^{t+1} - \nabla f(x^{t+1})}^2}} \\
    &=\frac{1}{n^2 \probavailable}\sum_{i=1}^n \ExpSub{h}{\norm{k^{t+1}_i - \ExpSub{h}{k^{t+1}_i}}^2} \\
...
\begin{proof}
  Clearly, this is holds for \algname{DASHA}-\algname{PP}. Next, we show it for \algname{DASHA}-\algname{PP}-\algname{PAGE}:
  \begin{align*}
    &\ExpSub{h}{k^{t+1}_i} \\
    &= \probpage \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \frac{b}{\probpage} \left(h^t_i - \nabla f_i(x^{t})\right)\right) \\
    &\quad + (1 - \probpage) \ExpSub{h}{\frac{1}{B}\sum_{j \in I^t_i}\left(\nabla f_{ij}(x^{t+1}) - \nabla f_{ij}(x^{t})\right)} \\
    &= \probpage \left(\nabla f_i(x^{t+1}) - \nabla f_i(x^{t}) - \frac{b}{\probpage} \left(h^t_i - \nabla f_i(x^{t})\right)\right) \\
