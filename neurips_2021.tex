...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\quad + (1 - a)^2\norm{g^{t} - h^{t}}^2.\\
  \end{align*}
  It is left to consider that $\probpairaa \leq \probavailable^2$ to prove \eqref{eq:compressor_global_error}. The second inequality can be proved almost in the same way:
  \begin{align*}
    &\ExpSub{\cC}{\ExpSub{\probavailable}{\norm{g^{t+1}_i - h^{t+1}_i}^2}} \\
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\quad + \left(1 - b\right)^2 \norm{h^{t} - \nabla f(x^{t})}^2. \\
  \end{align*}
  Using inequality $\probpairaa \leq \probavailable^2$, we have
  \begin{align*}
